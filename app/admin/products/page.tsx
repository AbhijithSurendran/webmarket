        ...
        <div className="p-6 lg:p-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">Manage your catalog of {items.length} item{items.length !== 1 ? "s" : ""}</p>
                </div>
                <Link href="/admin/products/new" className="btn-primary text-sm py-2.5 px-5 shadow-md hover:shadow-lg transition-all">
            ...
            {items.length === 0 ? (
                <div className="admin-card p-16 text-center flex flex-col items-center justify-center">
                    ...
                    <h3 className="text-lg font-heading font-semibold text-gray-900 mb-2">No products added yet</h3>
                    <p className="text-gray-500 mb-6 max-w-sm">Get started by creating your first product to display on the public website.</p>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm min-w-[700px]">
                            <thead className="bg-gray-50 border-b border-gray-100">
                                <tr>
                                    <th className="text-left px-6 py-4 text-gray-500 font-semibold tracking-wide text-xs uppercase">Product Name</th>
                                    <th className="text-left px-6 py-4 text-gray-500 font-semibold tracking-wide text-xs uppercase hidden sm:table-cell">Price</th>
                            ...
                            <tbody className="divide-y divide-gray-50 bg-white">
                                {items.map((item) => (
                                    <tr key={item.id} className="hover:bg-gray-50/50 transition-colors group">
                                        ...
                                        <td className="px-6 py-4 hidden sm:table-cell font-medium text-gray-700">
                                            {item.price ? `$${item.price.toFixed(2)}` : <span className="text-gray-400 italic">Not set</span>}
                                        </td>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Link href={`/admin/products/${item.id}`} className="btn-ghost py-1.5 px-3 text-xs bg-white border border-gray-200 hover:border-gray-300 hover:bg-gray-50 shadow-sm text-gray-700">
